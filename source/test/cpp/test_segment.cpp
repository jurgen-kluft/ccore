...
UNITTEST_SUITE_BEGIN(segment)
{
    UNITTEST_FIXTURE(operations)
    {
        ...
        UNITTEST_TEST(initialization)
        {
            ...
            nsegment::initialize(&allocator, 4u * 1024 * 1024, 1, 65536);

            // Check that top-level free list is initialized (head index should not be cINVALID_INDEX)
            const u32 top_level = (u32)math::ilog2(65536 / 1);
            CHECK(allocator.m_free_list_heads[top_level] != ~0u);
        ...
        UNITTEST_TEST(initialization_and_teardown)
        {
            ...
            CHECK(allocator.m_segment_min_pages == 1);
            CHECK(allocator.m_segment_max_pages == 65536);
            CHECK(allocator.m_offsets != nullptr);
            CHECK(allocator.m_flags != nullptr);
            CHECK(allocator.m_chain != nullptr);
            CHECK(allocator.m_list != nullptr);
        ...
        UNITTEST_TEST(initialization_and_N_alloc_deallocs)
        {
            ...
            nsegment::node_t node1 = nsegment::alloc_node(&allocator, page_size);
            CHECK(node1 != nsegment::INVALID_NODE);

            // Test 2: allocate another small node
            nsegment::node_t node2 = nsegment::alloc_node(&allocator, page_size);
            CHECK(node2 != nsegment::INVALID_NODE);
            CHECK(node1 != node2);  // should be different nodes

            // Test 3: allocate a larger node (4 pages)
            nsegment::node_t node3 = nsegment::alloc_node(&allocator, (u64)4 * page_size);
            CHECK(node3 != nsegment::INVALID_NODE);

            // Test 4: allocate maximum sized node
            nsegment::node_t node4 = nsegment::alloc_node(&allocator, (u64)65536 * page_size);
            CHECK(node4 != nsegment::INVALID_NODE);

            // Test 5: oversized allocation should fail
            nsegment::node_t node_fail = nsegment::alloc_node(&allocator, (u64)(65536 + 1) * page_size);
            CHECK(node_fail == nsegment::INVALID_NODE);

            // Test 6: undersized allocation (smaller than min)
            nsegment::node_t node_tiny = nsegment::alloc_node(&allocator, 1);
            CHECK(node_tiny != nsegment::INVALID_NODE);  // should round up to min page size
        ...
        UNITTEST_TEST(split_path_allocation)
        {
            ...
            nsegment::allocator_t allocator;
            nsegment::initialize(&allocator, 1024 * 1024, 1, 65536);  // smaller address space
            const u32 page_size = (u32)v_alloc_get_page_size();
        ...
        {
            // Test: allocation exhaustion returns INVALID_NODE
            nsegment::allocator_t allocator;
            nsegment::initialize(&allocator, 4, 1, 4);  // very small address space (4 pages total)
            const u32 page_size = (u32)v_alloc_get_page_size();

            // Allocate all available space
            nsegment::node_t node1 = nsegment::alloc_node(&allocator, (u64)4 * page_size);
            CHECK(node1 != nsegment::INVALID_NODE);
        ...
        UNITTEST_TEST(stress_mixed_sizes_and_exhaustion)
        {
            ...
            {
                nsegment::allocator_t allocator;
                nsegment::initialize(&allocator, address_space_pages, 1, max_segment_pages);
                ...
                for (u32 i = 0; i < 32; ++i)
                {
                    const u32 head = allocator.m_free_list_heads[i];
            ...
            // Deterministic exact-capacity check with min-size allocations.
            {
                ...
                CHECK(count == address_space_pages);
        ...
        UNITTEST_TEST(stress_large_first_sweeps)
        {
            ...
            for (u32 round = 0; round < 20; ++round)
            {
                ...
                CHECK(tail == nsegment::INVALID_NODE);
