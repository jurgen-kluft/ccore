...
UNITTEST_SUITE_BEGIN(duomap1)
{
    UNITTEST_FIXTURE(duomap12)
    {
        ...
        UNITTEST_TEST(lazy_free)
        {
            ...
            g_memclr(&bin00, sizeof(bin00));
            g_memclr(&bin01, sizeof(bin01));
            g_memclr(bin1, sizeof(bin1));

            const u32 maxbits = 512;

            u64 expected_bin00 = D_U64_MAX;
            u64 expected_bin01 = D_U64_MAX;
            u64 expected_bin1  = D_U64_MAX;

            nduomap12::setup_free_lazy(&bin00, &bin01, bin1, maxbits);

            CHECK_EQUAL(expected_bin00, bin00);
            for (u32 i = 0; i < maxbits; i++)
            {
                nduomap12::tick_free_lazy(&bin00, &bin01, bin1, maxbits, i);
                if ((i & 63) == 0)
                {
                ...
                CHECK_EQUAL(expected_bin00, bin00);
                CHECK_EQUAL(expected_bin01, bin01);
        ...
        UNITTEST_TEST(set_and_is_set)
        {
            ...
            for (u32 i = 0; i < maxbits; i++)
            {
                ...
                expected_bin10 = (u64)((expected_bin10 << 1) | 1);
                if ((i & 63) == 63)
                {
        ...
        UNITTEST_TEST(clr_and_is_clr)
        {
            ...
            for (u32 i = 0; i < maxbits; i++)
            {
                ...
                CHECK_EQUAL(expected_bin00, bin00);
                CHECK_EQUAL(expected_bin10, bin01);
                CHECK_FALSE(nduomap12::get(&bin00, &bin01, bin1, maxbits, i));
            }
        }
        ...
        UNITTEST_TEST(find0_and_set)
        {
            ...
            CHECK_EQUAL((s32)0, nduomap12::find0(&bin00, &bin01, bin1, maxbits));
            for (s32 i = 0; i < maxbits; i++)
            {
                CHECK_EQUAL((s32)i, nduomap12::find0_and_set(&bin00, &bin01, bin1, maxbits));
            }
        }
    }
}
