...
namespace ncore
{
    void bin_setup(bin32_t* bin, u16 item_size, u32 max_items)
    {
        ...
        bin->m_items_count            = 0;
        bin->m_items_capacity         = (u32)(narena::reserved_size(bin->m_items) / item_size);
    ...
    u32 bin_capacity(bin32_t const* bin) { return bin->m_items_capacity; }
    u32 bin_highwater_mark(bin32_t const* bin) { return bin->m_items_free_index; }
    ...
    void* bin_alloc(bin32_t* bin)
    {
        ...
        else
        {
            ...
            {
                case 3:
                    narena::commit(bin->m_bin, (bin->m_bin_level2_offset + ((bin->m_items_free_index + 1 + 63) >> 12)) * sizeof(u64));
                    narena::commit(bin->m_bin3, ((bin->m_items_free_index + 1 + 63) >> 6) * sizeof(u64));
                    break;
                case 2:
                    narena::commit(bin->m_bin, (bin->m_bin_level2_offset + ((bin->m_items_free_index + 1 + 63) >> 6)) * sizeof(u64));
                    // we have no bin3
                    break;
            }
            ...
            switch (bin->m_bin_level_count)
            {
                case 3: nbinmap24::tick_lazy(bm0, bm1, bm2, bm3, items_capacity, bin->m_items_free_index); break;
                case 2: nbinmap18::tick_lazy(bm0, bm1, bm2, items_capacity, bin->m_items_free_index); break;
                case 1: nbinmap12::tick_lazy(bm0, bm1, items_capacity, bin->m_items_free_index); break;
                case 0: break;
            }

            bin->m_items_free_index++;
            bin->m_items_count += 1;

            // Inform the arena of the required commitment for the item memory, so that it can make sure it's committed.
            narena::commit(bin->m_items, bin->m_items_free_index * bin->m_item_sizeof);

            return items + ((bin->m_items_free_index - 1) * bin->m_item_sizeof);
        }
    }
    ...
    void bin_free(bin32_t* bin, void* ptr)
    {
        ...
        switch (bin->m_bin_level_count)
        {
            case 3: nbinmap24::clr(bm0, bm1, bm2, bm3, bin->m_items_free_index, item_index); break;
            case 2: nbinmap18::clr(bm0, bm1, bm2, bin->m_items_free_index, item_index); break;
    ...
    s32 bin_highest_free(bin32_t const* bin)
    {
        ...
        switch (bin->m_bin_level_count)
        {
            case 3: hi = nbinmap24::find_last(bm0, bm1, bm2, bm3, items_max); break;
            case 2: hi = nbinmap18::find_last(bm0, bm1, bm2, items_max); break;
    ...
    u32 bin_capacity(bin16_t const* bin) { return bin->m_items_capacity; }
    u32 bin_highwater_mark(bin16_t const* bin) { return bin->m_items_free_index; }
    ...
    void* bin_alloc(bin16_t* bin)
    {
        ...
        else
        {
            ...
            switch (bin->m_bin_level_count)
            {
                case 2: narena::commit(bin->m_bin, 1 + 16 + ((bin->m_items_free_index + 1 + 63) >> 6)); break;
            }

            switch (bin->m_bin_level_count)
            {
                case 2: nbinmap18::tick_lazy(bm0, bm1, bm2, bin->m_items_capacity, bin->m_items_free_index); break;
