...
namespace ncore
{
    ...
    struct arena_t
    {
        ...
        void* alloc(int_t size, s32 alignment);           // allocate 'size' from the reserved region with the given alignment
        void* alloc_and_zero(int_t size);                 // allocate 'size' from the reserved region
        void* alloc_and_zero(int_t size, s32 alignment);  // allocate 'size' from the reserved region with the given alignment
    ...
    public:
        ...
        virtual void* v_allocate(u32 size, u32 alignment) { return m_vmem->alloc((int_t)size, alignment); }
